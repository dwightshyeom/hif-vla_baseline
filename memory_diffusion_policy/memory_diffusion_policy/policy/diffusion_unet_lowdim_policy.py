...
class DiffusionUnetLowdimPolicy(BaseLowdimPolicy):
    def __init__(self, 
            ...
            # Observation projection: reduce high-dim obs via learnable MLP
            use_obs_projection=False,
            obs_projection_output_dim=20,
            obs_projection_num_layers=2,
            raw_obs_dim=None,
        ...
        if use_obs_projection:
            assert raw_obs_dim is not None, \
                "raw_obs_dim must be set when use_obs_projection=True"
            layers = []
            in_dim = raw_obs_dim
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        nobs = self.normalizer['obs'].normalize(obs_dict['obs']) #obs_dict['obs'] #
    ...
    def compute_loss(self, batch):
        ...
        obs = nbatch['obs'] #batch['obs'] #
        action = nbatch['action']
        if self.obs_projection is not None:
            obs = self.obs_projection(obs)
        # print("#$$$$$$$$$$$$$$$$$$$$$$#")
        # raise ValueError("Invalid observation or action")
        ...
        if self.obs_as_local_cond:
            ...
            # raise ValueError("Debugging observation or action")
            if self.pred_action_steps_only:
                To = self.n_obs_steps
                start = To
                if self.oa_step_convention:
        ...
        # print("noise_trajectory.shape: ", noisy_trajectory.shape)
        # print("====================== END ======================")
        # raise ValueError("Debugging observation or action")
