...
from memory_diffusion_policy.env.pusht.pusht_image_two_swap_env import PushTImageTwoSwapEnv
...
# Mirrors the three-goals image runner: same trajectory-drawing primitives so
# that past-action PDFs look identical across tasks.
# ---------------------------------------------------------------------------
_CANVAS_SIZE = 512
_AGENT_RADIUS = 15.0
...
class PushTImageTwoSwapRunner(BaseImageRunner):
    """
    Env runner for the two-block swap Push-T task (image observations).

    Mirrors PushTImageThreeGoalsRunner's structure and polymorphic hooks:
      - vanilla DP policies just need predict_action()
      - LSTM+DP policies can additionally implement
        predict_past_actions() and update_lstm_with_intermediates() to
        unlock past-action PDFs and intermediate-obs feeding respectively.
    """
    ...
    def __init__(self,
                 ...
                 n_envs=None,
                 rollout_mode="wandb",
                 blue_start_side='random'):
        """
        blue_start_side:
            'left'   - Blue always spawns in Target A (matches single-side demos)
            'right'  - Blue always spawns in Target B
            'random' - seed-based 50/50 draw per env. Demos include both
                       sides, so use 'random' for balanced eval.

        rollout_mode:
            "wandb"  — default. Spawns n_envs (or n_train+n_test) parallel envs
                       at once and uploads rendered videos to wandb.
        ...
        assert blue_start_side in ('left', 'right', 'random'), \
            f"blue_start_side must be 'left'|'right'|'random', got {blue_start_side!r}"

        steps_per_render = max(10 // fps, 1)

        # Constructor-time side only matters when blue_start_side != 'random';
        # otherwise the per-env init_fn overrides it via set_blue_start_side().
        ctor_side = blue_start_side if blue_start_side != 'random' else 'left'

        def env_fn():
            return MultiStepWrapper(
                VideoRecordingWrapper(
                    PushTImageTwoSwapEnv(
                        legacy=legacy_test,
                        render_size=render_size,
                        blue_start_side=ctor_side,
                    ),
        ...
        def _resolve_side(seed_):
            if blue_start_side == 'random':
                # Deterministic per-seed draw so runs are reproducible.
                return 'left' if np.random.RandomState(seed_).random() < 0.5 else 'right'
            return blue_start_side

        # train
        for i in range(n_train):
            seed = train_start_seed + i
            enable_render = i < n_train_vis
            side = _resolve_side(seed)

            def init_fn(env, seed=seed, enable_render=enable_render, side=side):
                ...
                # MultiStepWrapper -> VideoRecordingWrapper -> PushTImageTwoSwapEnv
                env.env.env.set_blue_start_side(side)
    ...
    def run(self, policy: BaseImagePolicy, epoch: int = 0):
        ...
        for chunk_idx in range(n_chunks):
            ...
            # Per-env action history + random sampling of viz targets.
            ...
            pbar = tqdm.tqdm(total=self.max_steps,
                desc=f"Eval PushTImageTwoSwap {chunk_idx+1}/{n_chunks}",
            ...
            while not done:
                ...
                # Set swap-progress overlay on video BEFORE stepping so
                # VideoRecordingWrapper captures frames with the indicator.
                # Shows per-block coverage of its swap target.
                def _set_progress_overlay(env_wrapper):
                    inner = env_wrapper.env.env  # VideoRecordingWrapper -> PushTImageTwoSwapEnv
                    cov_b, cov_r = inner._compute_swap_coverages()
                    env_wrapper.env.step_based_overlay = f"B:{cov_b:.2f} R:{cov_r:.2f}"
                    return None
            ...
            # folder and free per-chunk caches before the next chunk starts.
            # See class docstring on rollout_mode for the rationale.
            if self.rollout_mode == "local":
        ...
        for i in range(n_inits):
            ...
            # Reward is clip(0.5 * (cov_blue + cov_red) / threshold, 0, 1).
            # It saturates at 1.0 once the joint coverage is close to done, so
            # we use >= 1.0 as the binary success signal (matches three-goals).
