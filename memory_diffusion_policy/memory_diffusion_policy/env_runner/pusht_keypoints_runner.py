...
class PushTKeypointsRunnerWithGoal(BaseLowdimRunner):
    def __init__(self,
            output_dir,
            keypoint_visible_rate=1.0,
            n_train=10,
            ...
            n_envs=None,
            randomize_goal=True,
            include_goal_keypoint=True  # If True, prepend goal keypoints to obs to match training format
        ):
        """
        Args:
            include_goal_keypoint: If True, prepends goal_keypoint to observation.
                This should match how the training data was prepared:
                obs = [goal_keypoint (18D), block_keypoint (18D), agent_pos (2D)]
                Set to True when using PushTLowdimDatasetWithGoal.
        """
        super().__init__(output_dir)
    ...
    def run(self, policy: BaseLowdimPolicy):
        ...
        for chunk_idx in range(n_chunks):
            ...
            if self.include_goal_keypoint:
                ...
                # MultiStepWrapper -> VideoRecordingWrapper -> PushTKeypointsEnv
                def get_goal_kp_fn(env):
                    # Navigate through wrappers to get base PushTKeypointsEnv
                    base_env = env.env.env  # unwrap twice
                    return base_env._get_goal_keypoints()
                
                goal_kp_fn_dill = dill.dumps(get_goal_kp_fn)
                goal_keypoints_list = env.call('run_dill_function', goal_kp_fn_dill)
                goal_keypoints = np.array(goal_keypoints_list)  # (B, 9, 2)

            pbar = tqdm.tqdm(total=self.max_steps, desc=f"Eval PushtKeypointsRunnerWithGoal {chunk_idx+1}/{n_chunks}", 
                leave=False, mininterval=self.tqdm_interval_sec)
            done = False
            while not done:
                Do = obs.shape[-1] // 2
                ...
                if self.include_goal_keypoint:
                    # Use the goal keypoints we got at reset (constant for episode)
                    # Reshape and repeat goal keypoints for all timesteps in observation window
                    goal_keypoints_flat = goal_keypoints.reshape(goal_keypoints.shape[0], -1)  # (B, 18)
                    goal_keypoints_seq = np.repeat(
                        goal_keypoints_flat[:, None, :], 
                        self.n_obs_steps, 
                        axis=1
                    )  # (B, T, 18)
                    
                    # Concatenate: [goal_keypoint, block_keypoint, agent_pos]
                    # block_keypoints already contains [block_kps + agent_pos] from env
                    # We need to prepend goal_keypoints
                    obs_with_goal = np.concatenate([
                        goal_keypoints_seq,
                        block_keypoints
                    ], axis=-1)  # (B, T, 18 + Do)
                    
                    # Create mask for goal keypoints (always visible)
                    goal_mask = np.ones((obs_mask.shape[0], obs_mask.shape[1], 18), dtype=bool)
                    obs_mask_with_goal = np.concatenate([goal_mask, obs_mask], axis=-1)
                    
                    # create obs dict
                    np_obs_dict = {
                        'obs': obs_with_goal,
                        'obs_mask': obs_mask_with_goal
                    }
                else:
