...
class PushTKeypointsFrictionRunner(BaseLowdimRunner):
    """
    Env runner for the Push-T friction task (keypoint observations).

    Uses PushTKeypointsFrictionEnv's flat obs vector split into
    (obs, obs_mask) halves — same convention as every other PushT keypoint
    runner in this repo. Per-episode friction-side randomization happens
    inside the env itself (driven by ``self._seed``); no init_fn work needed.
    """
    ...
    def __init__(self,
        ...
        # Regenerate the keypoint template with the friction env's (smaller)
        # block/agent scale so env keypoints and any downstream dataset
        # keypoints use the same 9-point layout.
    ...
    def run(self, policy: BaseLowdimPolicy):
        ...
        for chunk_idx in range(n_chunks):
            ...
            pbar = tqdm.tqdm(total=self.max_steps,
                desc=f"Eval PushTKeypointsFriction {chunk_idx+1}/{n_chunks}",
