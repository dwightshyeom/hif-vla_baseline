"""
Env runner for the THREE-TRACK Push-T friction task (keypoint observations).

Mirrors PushTKeypointsFrictionRunner. Per-episode 3-track layout
(divider_1_x, divider_2_x, normal_lane_idx, block / agent spawn) is
randomized inside the env from ``self._seed``; the runner just hands each
env a distinct seed.
"""
import wandb
import numpy as np
...
class PushTKeypointsFrictionThreeTracksRunner(BaseLowdimRunner):
    def __init__(self,
                 output_dir,
        ...
        # Same scaled keypoint template as the 2-track friction env (the
        # 3-track subclass inherits DEFAULT_BLOCK_SCALE / DEFAULT_AGENT_RADIUS).
        kp_kwargs = PushTKeypointsFrictionThreeTracksEnv.genenerate_keypoint_manager_params()
            ...
            return MultiStepWrapper(
                VideoRecordingWrapper(
                    PushTKeypointsFrictionThreeTracksEnv(
                        ...
                        include_memory_flags=include_memory_flags,
        ...
        self.include_memory_flags = include_memory_flags
    ...
    def run(self, policy: BaseLowdimPolicy):
        ...
        for chunk_idx in range(n_chunks):
            ...
            pbar = tqdm.tqdm(total=self.max_steps,
                desc=f"Eval PushTKeypointsFrictionThreeTracks {chunk_idx+1}/{n_chunks}",
