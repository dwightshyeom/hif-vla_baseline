...
from memory_diffusion_policy.env.pusht.pusht_image_friction_three_tracks_env import PushTImageFrictionThreeTracksEnv
...
# ---------------------------------------------------------------------------
# Past-action PDF helpers — identical primitives to the three-goals / swap
# image runners so viz styling stays consistent across tasks.
# ---------------------------------------------------------------------------
_CANVAS_SIZE = 512
_AGENT_RADIUS = 15.0
...
def _render_past_action_pdf(agent_pos, action_history, past_pred,
                            seed, step, epoch, output_dir, env_image=None):
...
class PushTImageFrictionThreeTracksRunner(BaseImageRunner):
    """
    Env runner for the Push-T friction (memory) task (image observations).

    Mirrors the three-goals / swap image runners: polymorphic on policy
    capabilities — past-action PDFs when the policy exposes
    ``predict_past_actions()``, intermediate-obs feeding when
    ``_feed_intermediate_obs`` is True.

    The env itself randomizes ``high_friction_side`` in ``reset()`` from
    ``self._seed``, so each env in the vector already gets a distinct
    left/right friction layout without any explicit per-env init_fn work.
    """
        ...
        def env_fn():
            return MultiStepWrapper(
                VideoRecordingWrapper(
                    PushTImageFrictionThreeTracksEnv(
                        legacy=legacy_test,
    ...
    def run(self, policy: BaseImagePolicy, epoch: int = 0):
        ...
        for chunk_idx in range(n_chunks):
            ...
            pbar = tqdm.tqdm(total=self.max_steps,
                desc=f"Eval PushTImageFrictionThreeTracks {chunk_idx+1}/{n_chunks}",
            ...
            while not done:
                ...
                # Progress overlay on recorded video: "T:<trial>/<max>  C:<cov>".
                def _set_progress_overlay(env_wrapper):
                    inner = env_wrapper.env.env  # VideoRecordingWrapper -> PushTImageFrictionThreeTracksEnv
                    cov = inner._compute_coverage()
                    trial = int(getattr(inner, 'current_trial', 0))
                    max_t = int(getattr(inner, 'max_trials', 1))
                    tag = "R" if getattr(inner, '_return_phase', False) else "P"
                    env_wrapper.env.step_based_overlay = (
                        f"{tag} T:{trial}/{max_t} C:{cov:.2f}"
                    )
                    return None
        ...
        for i in range(n_inits):
            ...
            # The friction env emits reward=1.0 only on success (coverage >
            # threshold in a non-return phase); otherwise 0. So max_reward
            # == 1.0 is a clean binary success signal.
