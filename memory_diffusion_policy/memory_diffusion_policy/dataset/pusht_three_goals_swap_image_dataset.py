...
class PushTThreeGoalsSwapImageDataset(BaseImageDataset):
    """
    Vanilla-DP image dataset for the three-goals swap Push-T task.
    ...
    Observation returned to the policy is only {image, agent_pos (2D)}.
    The image already encodes the three fixed green targets and the two
    colored blocks, so the policy can learn the swap protocol from images
    alone (matching the two-swap image dataset interface).
    ...
    def get_normalizer(self, mode='limits', **kwargs):
        ...
        # only on agent_pos so it matches what the policy receives.
        data = {
