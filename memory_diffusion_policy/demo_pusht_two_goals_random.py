...
def main(output, render_size, control_hz, start_seed):
    """
    Collect demonstration for the Push-T Two Goals RANDOM task.
    
    Usage: python demo_pusht_two_goals_random.py -o data/pusht_random_two_goals_demo.zarr
           python demo_pusht_two_goals_random.py -o data/pusht_random_two_goals_demo.zarr -s 43  # Start from seed 43
    ...
    Task: Push the T block to overlap with BOTH goal areas (can be completed in any order):
          - Goal positions are RANDOMIZED on each episode
    ...
    - goal_1_keypoint (9, 2): target keypoints for Goal 1 (randomized each episode)
    - goal_2_keypoint (9, 2): target keypoints for Goal 2 (randomized each episode)
    ...
    # create PushT two goals RANDOM env with keypoints
    kp_kwargs = PushTKeypointsTwoGoalsRandomEnv.genenerate_keypoint_manager_params()
    env = PushTKeypointsTwoGoalsRandomEnv(
    ...
    print("\n" + "="*60)
    print("Push-T Two Goals RANDOM Data Collection")
    print("="*60)
    print("Task: Visit BOTH goal areas (can be in any order)")
    print("Note: Goal positions are RANDOMIZED on each episode")
    ...
    while True:
        ...
        print(f'  Goal 1 (GREEN - {goal_1_side}) at: ({env.goal_pose_1[0]:.1f}, {env.goal_pose_1[1]:.1f}, θ={env.goal_pose_1[2]:.2f})')
        print(f'  Goal 2 (GREEN - {goal_2_side}) at: ({env.goal_pose_2[0]:.1f}, {env.goal_pose_2[1]:.1f}, θ={env.goal_pose_2[2]:.2f})')
        ...
        while not done:
            ...
            if not act is None:
                ...
                # goal keypoints for both goals (randomized each episode but fixed during episode)
