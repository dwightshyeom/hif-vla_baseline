...
from prismatic.models.action_heads import  MotionTokenManager, JointExpert
...
class FinetuneConfig:
    ...
    # Dataset
    data_root_dir: Path = Path("")      # Directory containing RLDS datasets
    ...
    max_steps: int = 150_005                         # Max number of training steps
    ...
    run_id_note: Optional[str] = 1                # Extra note to add to end of run ID for logging
    run_id_override: Optional[str] = None            # Optional string to override the run ID with
    wandb_log_freq: int = 10                         # WandB logging frequency in steps

    # fmt: on
    history_length: int = 8                          # Length of motion history
...
def run_forward_pass(
    ...
    motion_hidden_states = output.hidden_states[-1][:, num_patches+1:num_patches+1+NUM_ACTIONS_CHUNK].reshape(batch["input_ids"].shape[0], NUM_ACTIONS_CHUNK, -1)
    last_hidden_states = output.hidden_states[-1]  # (B, seq_len, D)
    text_hidden_states = last_hidden_states[:, num_patches+NUM_ACTIONS_CHUNK:-1]
    ...
    with torch.autocast("cuda", dtype=torch.bfloat16):
        his_motion_feature = motion_encoder(batch["mv_history"].to(device_id))  # [B, 16, D]
    
    pre_action,res_motion = action_head(actions_hidden_states,motion_hidden_states.to(torch.bfloat16),time_cond=his_motion_feature)
...
def save_training_checkpoint(
    ...
    if distributed_state.is_main_process:
        ...
        torch.save(motion_encoder.state_dict(), checkpoint_dir / f"motion_encoder--{checkpoint_name_suffix}")
        torch.save(motion_manager.state_dict(),checkpoint_dir / f"motion_manager--{checkpoint_name_suffix}")
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    assert cfg.use_l1_regression, "Cannot do L1 regression. Please pick it!"
    ...
    if cfg.use_l1_regression:
       action_head = init_module(
           ...
           "action_head",
            cfg,
            device_id,
            {"depth":6, "input_dim":vla.module.llm_dim,"hidden_dim":vla.module.llm_dim//4,"action_dim":ACTION_DIM},
            to_bf16=True,
            )
    ...
    motion_encoder = init_module(
            ...
            cfg,
            device_id,
            {"in_channels": 2, "hidden_dim": vla.module.llm_dim//4, "out_dim": vla.module.llm_dim//4, "num_frames": cfg.history_length//2, "num_patches": 64},
    )

    use_extra_motion_token=True
    if use_extra_motion_token:
        ...
        motion_token = motion_manager.module.get_motion_token(cfg.batch_size)
    ...
    # trainable_params += [param for param in motion_decoder.parameters() if param.requires_grad]#%%%%%
    trainable_params += [param for param in motion_encoder.parameters() if param.requires_grad]#%%%%%
    if cfg.use_l1_regression:
        trainable_params += [param for param in action_head.parameters() if param.requires_grad]
    if cfg.use_proprio:
        trainable_params += [param for param in proprio_projector.parameters() if param.requires_grad]
    if use_extra_motion_token:
        trainable_params += [param for param in motion_manager.parameters() if param.requires_grad]
    ...
    with tqdm.tqdm(total=cfg.max_steps, leave=False) as progress:
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            loss, metrics = run_forward_pass(
                ...
                motion_token=motion_token if use_extra_motion_token else None,
                batch=batch,
                action_tokenizer=action_tokenizer,
                device_id=device_id,
                use_l1_regression=cfg.use_l1_regression,
            ...
            if gradient_step_idx > 0 and log_step % cfg.save_freq == 0:
                save_training_checkpoint(
                    ...
                    motion_encoder=motion_encoder,#%%%%%
                    proprio_projector=proprio_projector if cfg.use_proprio else None,
                    motion_manager=motion_manager if use_extra_motion_token else None,
            ...
            if cfg.use_val_set and log_step > 0 and log_step % cfg.val_freq == 0:
                run_validation(
                    vla=vla,#motion_decoder=motion_decoder,#%%%%%
                    action_head=action_head,
                    proprio_projector=proprio_projector if cfg.use_proprio else None,
