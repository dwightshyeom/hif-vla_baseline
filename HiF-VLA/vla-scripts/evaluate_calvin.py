...
from experiments.robot.robot_utils import (
    ...
    set_seed_everywhere,
    extract_motion_vectors_from_images,
)
# logger = logging.getLogger(__name__)

os.environ["FFMPEG_BINARY"] = "auto-detect"
os.environ["CALVIN_ROOT"] = "/liujiacheng/linminghui/github-hifvla-calvin/calvin"
...
# /fanyiguo/Minghui/openoft-calvin/ckpt/31f090d05236101ebfc381b61c674dd4746d4ce0+calvin_abc_rlds+b16+lr-0.0005+lora-r32+dropout-0.0--image_aug--6mmdit_4vit_8his--parallel_dec_decoder_2--motion_action_l1_head--8_acts_chunk--continuous_acts--3rd_person_img--proprio_state--90000_chkpt/
...
#################################################################################################################
    ...
    history_length: int = 8                        # history length
...
def evaluate_policy(model, env, eval_sr_path, eval_result_path, num_procs, procs_id, eval_dir, ep_len, num_sequences, task_name='test', enrich_lang=False, debug=False, his_len=8):
    ...
    for initial_state, eval_sequence in eval_sequences:
        result = evaluate_sequence(env, model, task_oracle, initial_state, eval_sequence, val_annotations, debug, eval_dir, sequence_i, ep_len, his_len)
...
def evaluate_sequence(env, model, task_checker, initial_state, eval_sequence, val_annotations, debug, eval_dir, sequence_i, ep_len, his_len):
    ...
    for subtask_i, subtask in enumerate(eval_sequence):
        success = rollout_hi3(env, model, task_checker, subtask, val_annotations, debug, eval_dir, subtask_i, sequence_i, ep_len, his_len)
        if success:  # return 5!!!!!!!!!!!!!!!!!!!
...
def get_mv_feat(replay_images,T_hist):
    if len(replay_images) == 1:
        mv_sequence = np.zeros((T_hist-1, 2, 16, 16), dtype=np.float32)
    elif len(replay_images) < T_hist:
        mv_sequence = extract_motion_vectors_from_images(replay_images, fps=6, num_frames=len(replay_images))
        if isinstance(mv_sequence, torch.Tensor):
            mv_sequence = mv_sequence.numpy()
        pad_len = T_hist - len(replay_images)
        if mv_sequence.shape[0] > 0 and pad_len > 0:
            first_mv = mv_sequence[:1]  # shape [1, 2, 16, 16]
            pad = np.repeat(first_mv, repeats=pad_len, axis=0)
            mv_sequence = np.concatenate((pad, mv_sequence), axis=0)  # [T_hist-1,2,16,16]
    else:
        mv_sequence = extract_motion_vectors_from_images(replay_images, fps=6, num_frames=T_hist)#[t,2,16,16
        if isinstance(mv_sequence, torch.Tensor):
            mv_sequence = mv_sequence.numpy()
    dev = DEVICE
    mv_sequence =torch.from_numpy(mv_sequence[None,...]).to(device=dev, dtype=torch.bfloat16)

    return mv_sequence

def rollout(env, model, task_oracle, subtask, val_annotations, debug, eval_dir, subtask_i, sequence_i, ep_len, his_len):
    if debug:
        print(f"{subtask} ", end="")
        time.sleep(0.5)
    obs = env.get_obs()
    ...
    T_hist = his_len+1  
    frame_buf = deque(maxlen=T_hist)

    for step in range(ep_len):

         
        frame_buf.append(obs['rgb_obs']['rgb_static'])

        if len(action_queue) == 0:
            his_motion_seq = get_mv_feat(list(frame_buf),T_hist)
            actions = model.step(obs, lang_annotation, step, his_motion_seq)
...
def rollout_hi3(env, model, task_oracle, subtask, val_annotations, debug, eval_dir, subtask_i, sequence_i, ep_len, his_len):
    if debug:
        print(f"{subtask} ", end="")
        time.sleep(0.5)
    ...
    for step in range(50):
        ...
        his_motion_seq = get_mv_feat(list(frame_buf),T_hist)
        action_buffers[0] = model.step(obs, lang_annotation, 0, his_motion_seq)  # 8个动作
        ...
        his_motion_seq = get_mv_feat(list(frame_buf),T_hist)
        action_buffers[1] = model.step(obs, lang_annotation, 1, his_motion_seq)
        ...
        his_motion_seq = get_mv_feat(list(frame_buf),T_hist)
        action_buffers[2] = model.step(obs, lang_annotation, 2, his_motion_seq)
        ...
        img_dict['static'].append(copy.deepcopy(obs['rgb_obs']['rgb_static']))
        img_dict['gripper'].append(copy.deepcopy(obs['rgb_obs']['rgb_gripper']))
        frame_buf.append(obs['rgb_obs']['rgb_static'])  # 追加到历史帧缓冲
...
def initialize_model(cfg: GenerateConfig):
    ...
    # Load model
    model = get_model(cfg)
    # motion_decoder = get_motion_decoder(cfg, code_dim=4)#%%%%
    ...
    motion_manager = get_motion_manager(cfg, model.llm_dim)
    ...
    motion_encoder = get_hismotion_encoder(cfg, in_channels=2, hidden_dim=model.llm_dim//4, out_dim=model.llm_dim//4, num_frames=cfg.history_length//2, num_patches=64)

    return model, action_head, proprio_projector, processor, motion_manager, motion_encoder
...
def main(cfg: GenerateConfig):
    ...
    model, action_head, proprio_projector, processor, motion_manager, motion_encoder = initialize_model(cfg)
    ...
    eva = DualSystemCalvinEvaluation(model, action_head, proprio_projector, processor, motion_manager, motion_encoder, use_x0_prediction=cfg.use_x0_prediction)
