...
def normalize_action_and_proprio(traj: Dict, metadata: Dict, normalization_type: NormalizationType):
    ...
    if normalization_type == NormalizationType.NORMAL:#标准化到均值为0，方差为1
        ...
        return traj
    #1% 分位点（q01）：表示有 1% 的样本小于它，99% 的样本大于它；99% 分位点（q99）：表示有 99% 的样本小于它，仅有 1% 的样本大于它
    elif normalization_type in [NormalizationType.BOUNDS, NormalizationType.BOUNDS_Q99]:#线性归一化【-1，1】、使用百分位值 [1%, 99%] 做 min-max，去除异常值影响
        for key, traj_key in keys_to_normalize.items():
