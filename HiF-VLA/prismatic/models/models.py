...
class ProprioProjector(nn.Module):
    ...
    def __init__(self, proprio_dim: int, hidden_size: int) -> None:
        ...
        self.mlp = nn.Sequential(
            nn.Linear(self.proprio_dim, self.hidden_size, bias=True),
            nn.GELU(),
            nn.Linear(hidden_size, hidden_size, bias=True),
        )
        # self.fc1 = nn.Linear(self.proprio_dim, self.hidden_size, bias=True)
    ...
    def forward(self, proprio: torch.Tensor = None) -> torch.Tensor:
        ...
        projected_features = self.mlp(proprio)
...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        self.positional_embedding = nn.Parameter(
                scale * torch.randn(future_action_window_size + past_action_window_size + 1 + future_action_window_size + 2, hidden_size))

        self.blocks = nn.ModuleList([
            DiTBlock(hidden_size, num_heads, mlp_ratio=mlp_ratio) for _ in range(depth)
        ])
        self.final_layer = FinalLayer(hidden_size, self.out_channels)
        self.proprio_projector = ProprioProjector(proprio_dim=8, hidden_size=hidden_size)
    ...
    def initialize_weights(self):
        ...
        nn.init.normal_(self.proprio_projector.mlp[0].weight, std=0.02)
        nn.init.normal_(self.proprio_projector.mlp[2].weight, std=0.02)
    ...
    def forward(self, x, t, z, y):
        ...
        y = self.proprio_projector(y)
        y = y.unsqueeze(dim=1)
        x = torch.cat((c,y, x), dim=1)                        # (N, T+1, D)
        x = x + self.positional_embedding                   # (N, T+1, D)
        for block in self.blocks:
            x = block(x)                                    # (N, T+1, D)
        x = self.final_layer(x)                             # (N, T+1, out_channels)
        return x[:, c.shape[1]+y.shape[1]:, :]     # (N, T, C)

    def forward_with_cfg(self, x, t, z, cfg_scale, y):
        """
        ...
        model_out = self.forward(combined, t, z, y)
