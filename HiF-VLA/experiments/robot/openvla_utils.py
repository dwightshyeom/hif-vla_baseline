...
def get_motion_decoder(cfg: Any, code_dim: int) -> MotionTransformerTokenizer:

    motion_decoder=build_motion_tokenizer(code_dim=code_dim).to(DEVICE)

    motion_decoder = motion_decoder.to(torch.bfloat16).to(DEVICE)
    motion_decoder.eval()

    # Find and load checkpoint
    checkpoint_path = find_checkpoint_file(cfg.pretrained_checkpoint, "motion_decoder")
    state_dict = load_component_state_dict(checkpoint_path)
    motion_decoder.load_state_dict(state_dict)

    return motion_decoder
def get_hismotion_encoder(cfg: Any, in_channels: int, hidden_dim: int, out_dim: int, num_frames: int, num_patches: int) -> HisMotionEncoder:
    ...
    motion_encoder = motion_encoder.to(torch.bfloat16).to(DEVICE)
    motion_encoder.eval()

    # Find and load checkpoint
    checkpoint_path = find_checkpoint_file(cfg.pretrained_checkpoint, "motion_encoder")
    state_dict = load_component_state_dict(checkpoint_path)
...
def get_motion_manager(cfg: Any, llm_dim: int) -> MotionTokenManager:

    motion_manager=MotionTokenManager(llm_dim=llm_dim).to(DEVICE)

    motion_manager = motion_manager.to(torch.bfloat16).to(DEVICE)
    motion_manager.eval()

    # Find and load checkpoint
    checkpoint_path = find_checkpoint_file(cfg.pretrained_checkpoint, "motion_manager")
    state_dict = load_component_state_dict(checkpoint_path)
    motion_manager.load_state_dict(state_dict)
...
def get_action_head(cfg: Any, llm_dim: int) -> Union[L1RegressionActionHead, DiffusionActionHead]:
    ...
    """
    # assert not (cfg.use_l1_regression and cfg.use_diffusion and cfg.use_cond_dit), "Cannot use both L1 regression and diffusion action head!"
    ...
    if cfg.use_l1_regression:
        ...
        action_head = JointExpert(depth=6, input_dim=llm_dim, hidden_dim=llm_dim//4, action_dim=ACTION_DIM)
    else:
        raise ValueError("Either use_l1_regression must be True")
...
def get_vla_action(
    cfg: Any,
    vla: torch.nn.Module,#motion_decoder: Optional[torch.nn.Module],#%%%%
    processor: Any,
    obs: Dict[str, Any],
    task_label: str,
